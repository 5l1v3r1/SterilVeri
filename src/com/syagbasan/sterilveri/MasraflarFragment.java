...
import com.syagbasan.sterilveri.db.MasraflarlDB;
...
import com.syagbasan.sterilveri.masraflarfragmentlistviews.CustomListViewAdapter;
import com.syagbasan.sterilveri.masraflarfragmentlistviews.RowItem;
...
public class MasraflarFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		rootView = inflater.inflate(R.layout.masraflar_fragment, container, false);
		...
		adapter = new CustomListViewAdapter(getActivity(),R.layout.masraflar_fragment_list_item, PersonalrowItems);
			...
			public boolean onItemLongClick(AdapterView<?> parent, View view,
	                ...
	                public void onClick(DialogInterface dialog2, int id) {
	                	...
	                	builder.setPositiveButton("Evet",
			                ...
			                public void onClick(DialogInterface dialog23, int id) {
								...
								for (int i = 0; i < entryMasraflarDB.getPersonKeyAllRowId(PersonalrowItems.get(position).getDate(),PersonalrowItems.get(position).getPerson(),number).length; i++) {
									if (entryMasraflarDB.getPersonKeyAllRowId(PersonalrowItems.get(position).getDate(),PersonalrowItems.get(position).getPerson(),number)[i] != null) {
										returned = entryMasraflarDB.deleteToDo(Long.parseLong(entryMasraflarDB.getPersonKeyAllRowId(PersonalrowItems.get(position).getDate(),PersonalrowItems.get(position).getPerson(),number)[i]));
									}			
								}
	            ...
	            builder1.setNeutralButton("Düzelt", new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog15, int which) {
						...
						UpdateRow(Long.parseLong(entryMasraflarDB.getPersonKeyAllRowId(PersonalrowItems.get(position).getDate(),PersonalrowItems.get(position).getPerson(),number)[0]),PersonalrowItems.get(position).getDate(),PersonalrowItems.get(position).getPerson(),number);
						RefreshPersonalList();
					}
				});
		...
		bVeriEkle.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				dialog.setContentView(R.layout.masraflar_add);
				dialog.show();				
				...
				if (entryMasraflarDB.getPerson().length!=0) {
					...
					public void onClick(View v) {
						...
						if (((etPerson.length()!=0 && cbNewPersonAdd.isChecked()) && etAmount.length()!=0) || (!cbNewPersonAdd.isChecked()&& spinner.getCount() != 0 && etAmount.length()!=0)) {
							...
							if (cbNewPersonAdd.isChecked()) {
								result = entryMasraflarDB.createEntry(currentDateandTime, etPerson.getText().toString(), etAmount.getText().toString());
							}else{
								result = entryMasraflarDB.createEntry(currentDateandTime, spinner.getSelectedItem().toString(), etAmount.getText().toString());
							}
		...
		bRemoveAllData.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (!PersonalrowItems.isEmpty()) {
					AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
					builder.setMessage("Tüm Masraflar verilerini silmek istediðinize emin misiniz?");
					builder.setCancelable(true);
					builder.setPositiveButton("Evet", new DialogInterface.OnClickListener() {
		                public void onClick(DialogInterface dialog2, int id) {
		                	entryMasraflarDB.deleteAllData();
		                	Toast.makeText(getActivity(), "Tüm Masraflar Verileri Silindi", Toast.LENGTH_SHORT).show();
	                		if (loaded) {
								soundPool.play(negativeSound, 0.5f,0.5f, 1, 0, 1f);
							}
		    				RefreshPersonalList();
	...
	private void ListViewContentConfig() {
		...
		RowItemThree item2 = new RowItemThree("Tarih","Masraf Adý", "Masraf Miktarý" );		
	...
	public void RefreshPersonalList(){
		adapter.clear();
		String[] dataPerson = entryMasraflarDB.getPerson();
		String[] dataDate = entryMasraflarDB.getDate();
		String[] dataAmount = entryMasraflarDB.getAmount();
	...
	public void onResume() {
		// TODO OnResume
		super.onResume();
		entryMasraflarDB = new MasraflarlDB(getActivity());
		entryMasraflarDB.open();
		RefreshPersonalList();
	};
	...
	private void UpdateRow(final long RowID,String date,String person,String amount) {
		...
		dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
		dialog.setCancelable(true);
		dialog.setContentView(R.layout.masraflar_fix);
		dialog.show();					
		...
		bOk.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (((etPerson.length()!=0  && etAmount.length()!=0) || ( spinner.getCount() != 0 && etAmount.length()!=0))) {
					...
					result = entryMasraflarDB.updateEntry(currentDateandTime, etPerson.getText().toString(), etAmount.getText().toString(),String.valueOf(RowID));
